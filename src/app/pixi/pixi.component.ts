...
import { Application, Container, Loader, LoaderResource, Sprite, Texture } from 'pixi.js';
...
export class PIXIComponent implements OnInit, OnDestroy {
  ...
  gif ='https://media1.giphy.com/media/46hpy8xB3MiHfruixn/giphy-downsized-large.gif';

  @Input()
  public devicePixelRatio = window.devicePixelRatio || 1;
  public lists
  constructor(private elementRef: ElementRef, private ngZone: NgZone, private characterService: CharacterService, private shutterstockService: ShutterstockService) {
  ...
  init() {
    this.shutterstockService.getSprites()
    .then(res => {
    ...
    this.loader.add("https://media1.giphy.com/media/46hpy8xB3MiHfruixn/giphy-downsized-large.gif",this.loadOption);
    this.loader.add("https://img1.picmix.com/output/stamp/normal/2/6/0/2/982062_c8ccb.gif", this.loadOption);
    ...
    this.loader.load((progress, resources)=>{
      this.addGif("https://media1.giphy.com/media/46hpy8xB3MiHfruixn/giphy-downsized-large.gif", resources);
  ...
  addCharacter(name:string, texture:string, owner:string) {
    let x = Math.floor(Math.random() * this.app.screen.width);
    let y = Math.floor(Math.random() * this.app.screen.height);
    let character = new Character(name, texture, owner, x, y);
  ...
  ngOnInit(): void {
    ...
    this.addCharacter("xelor", '../../assets/xel.jpg', 'golbian');
